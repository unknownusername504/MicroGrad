...
class CartPole:
    ...
    class SoftmaxPolicy:
        ...
        def __call__(self, model_output):
            action_probabilities = Softmax(model_output)().get_value()
    ...
    class AdaptiveSoftmaxPolicy:
        def __init__(self, epsilon=0.5, epsilon_decay=0.99, performance_threshold=0.5):
            ...
            self.softmax_policy = CartPole.SoftmaxPolicy()

        def __call__(self, model_output, performance):
            if np.random.uniform(0, 1) < self.epsilon:
                action = self.softmax_policy(model_output)
            else:
                action = model_output.argmax()
    ...
    def step(self, action):
        ...
        self.reward = Scalar(float(reward))
    ...
    def choose_action(self):
        ...
        action = self.policy(self.model_output, self.performance)
        return action
